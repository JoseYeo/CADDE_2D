# -*- coding: utf-8 -*-
"""
Created on Thu Jul 13 10:18:41 2023
...
for idx, t in enumerate(time[:-1]):
    ...
    # Predictor Step (using Forward Euler)
    z1p = z1[idx] + h*z2[idx]
    z2p = z2[idx] + h*f(t, z1[idx], z2[idx])
    
    # Corrector Step
    z1[idx+1] = z1[idx] + 0.5*h*(z2[idx] + z2p)
    z2[idx+1] = z2[idx] + 0.5*h*(f(t, z1[idx], z2[idx]) + f(time[idx+1], z1p, z2p))
